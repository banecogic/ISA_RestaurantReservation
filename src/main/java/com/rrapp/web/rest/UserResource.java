...
import com.rrapp.repository.AuthorityRepository;
import com.rrapp.repository.UserRepository;
import com.rrapp.security.AuthoritiesConstants;
import com.rrapp.service.MailService;
import com.rrapp.service.UserService;
import com.rrapp.web.rest.dto.ManagedUserDTO;
import com.rrapp.web.rest.dto.UserDTO;
import com.rrapp.web.rest.util.HeaderUtil;
import com.rrapp.web.rest.util.PaginationUtil;
...
public class UserResource {
    ...
    public ResponseEntity<?> createUser(@RequestBody ManagedUserDTO managedUserDTO, HttpServletRequest request) throws URISyntaxException {
        ...
        if (userRepository.findOneByLogin(managedUserDTO.getLogin()).isPresent()) {
            return ResponseEntity.badRequest()
                .headers(HeaderUtil.createFailureAlert("user-management", "userexists", "Korisnik sa datim imenom već postoji."))
                .body(null);
        } else if (userRepository.findOneByEmail(managedUserDTO.getEmail()).isPresent()) {
            return ResponseEntity.badRequest()
                .headers(HeaderUtil.createFailureAlert("user-management", "emailexists", "Email je već upotrebljen."))
                .body(null);
        } else {
    ...
    /**
     * GET  /users/search -> get users with search filtered by firstName and lastName
     */
    @RequestMapping(value = "/users/search/{searchCriteria}",
        method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<List<ManagedUserDTO>> getUserBySearchFilter(@PathVariable String searchCriteria) {
        log.debug("REST request to get Users with filtered by firstName and lastName through search filter : {}", searchCriteria);
        List<ManagedUserDTO> managedUserDTOs = userService.getUsersBySearchCriteria(searchCriteria).stream()
        		.map(user -> new ManagedUserDTO(user))
        		.collect(Collectors.toList());
     ...
     * 
     */
    @RequestMapping(value = "/users/sendfrienrequest/{login}/{friend}",
    		method = RequestMethod.POST,
    		produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<Void> sendFriendRequest(@PathVariable String login, @PathVariable String friend){
