    ...
    public void logout(HttpServletRequest request, HttpServletResponse response, Authentication authentication) {
        ...
        if (rememberMeCookie != null && rememberMeCookie.length() != 0) {
            try {
                ...
                log.info("Invalid cookie, no persistent token could be deleted", ice);		// Dodat drugi parametar, po sonar-u
            } catch (RememberMeAuthenticationException rmae) {
                log.debug("No persistent token found, so no token could be deleted", rmae);	// Dodat drugi parametar, po sonar-u
            }
        }
